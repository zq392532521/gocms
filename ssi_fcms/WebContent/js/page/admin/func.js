...
function operButton(){
	$.weeboxs.open('func_operButton.do?id='+$("#id").val(), {title:'管理操作按钮', contentType:'ajax',height:400,width:672,showOk:false,showButton:false,showOk:false,showCancel:false});
}
function hideButton(){
...
function parButton(){
	$.weeboxs.open('func_funcPar.do?id='+$("#id").val(), {title:'改变所属菜单', contentType:'ajax',height:400,width:300,showOk:false});
}
...
function setMenuicon(){
	...
	var tdHtml='<tr id="selectIconTr"><input type="hidden" value="" id="imgIconUrl"><td colspan="3" height="25" align="left" valign="middle" bgcolor="#FFFFFF" style="padding-left:7px"><td>';
	$('#selectIconTr').remove();
	switch($('#setMenuIcon').val()){
		case '2':
			RemoteButton=' <input type="file" name="fileField" id="fileField" />';
			tdHtml+=RemoteButton+'</td><td id="showImgIcion" style="display:none"></td><tr>';
			$('#setMenuIcon').after(tdHtml);
			break;
		case '3':
			localtdButton='<input type="button" id="choiceIconTr" onclick="selectImg()" value="选择服务器图标" class="button">';
			tdHtml+=localtdButton+'</td><td id="showImgIcion" style="display:none"></td>';
			$('#setMenuIcon').after(tdHtml);
		break;
	}
...
function selectImg(){
	...
	finder.type='Images';
	finder.basePath = checkParentBasePath+'js/ckfinder/ckfinder.js';	
...
function selectImgDo( fileUrl )
{
	...
	$("#showImgIcion").show();
	$("#showImgIcion").html("<a url='"+checkParentBasePath+fileUrl+"' target='_blank'>"+
							"<img src='"+checkParentBasePath+fileUrl+"' width='100' height='100' title='点击查看大图'/>"+
							"</a>");
	$("#imgIconUrl").val(checkParentBasePath+fileUrl);
}