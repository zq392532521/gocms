	...
	public String del(){
		try {
			if (channel!=null && channel.getId()!=null && channel.getId().trim().length()>0) {
				...
				log(getLoginName(), "删除栏目 "+channel.getName(), getHttpRequest());
	...
	public String editDo(){
		try {
			...
			if (channel.getId()!=null && channel.getId().trim().length()>0) {
				...
				log(getLoginName(), "更新栏目 "+channel.getName(), getHttpRequest());
			}else {
				//添加
				//判断页面标识是否存在
				if (channel.getPagemark()!=null && channel.getPagemark().trim().length()>0 && 
				...
				htmlquartzService.insert(htmlquartz);
				channelService.updateHtmlChannelJob(getServletContext(), site, channel, htmlquartz);
				log(getLoginName(), "添加栏目 "+channel.getName(), getHttpRequest());
			}
			return "makehtml";
		} catch (Exception e) {
				...
				if (channel.getId()!=null && channel.getId().trim().length()>0
					...
					if (parChannel!=null) {
						...
						if (channel!=null) {
							...
							channelService.update(channel);
							log(getLoginName(), "改变栏目 "+channel.getName()+" 的所属栏目为 "+parChannel.getName(), getHttpRequest());
							write("操作成功", "UTF-8");
						}
					}
					...
					if (oldchannel!=null && site!=null) {
						...
						log(getLoginName(), "改变栏目 "+oldchannel.getName()+" 的所属栏目为 "+site.getName()+" 站点的一级栏目", getHttpRequest());
						write("操作成功", "UTF-8");
					}
				}
			}
