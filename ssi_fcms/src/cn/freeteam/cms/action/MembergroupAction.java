	...
	public String editDo(){
		try {
			...
			if (membergroup.getId()!=null && membergroup.getId().trim().length()>0) {
				...
				log(getLoginName(), "更新会员组 "+membergroup.getName(), getHttpRequest());
			}else {
				//添加
				if (img!=null) {
					//生成目标文件
				...
				log(getLoginName(), "添加会员组 "+membergroup.getName(), getHttpRequest());
			}
		...
		if (membergroup!=null && membergroup.getId()!=null && membergroup.getId().trim().length()>0
			...
			if (membergroup!=null) {
				try {
					...
					log(getLoginName(), "会员组授权("+membergroup.getName()+" "+names+")", getHttpRequest());
					write("1", "UTF-8");
				} catch (Exception e) {
					log(getLoginName(), "会员组授权失败("+membergroup.getName()+" "+names+"):"+e.getMessage(), getHttpRequest());
