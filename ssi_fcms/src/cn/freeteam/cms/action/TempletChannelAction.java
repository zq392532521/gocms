	...
	public String del(){
		try {
			if (templetChannel!=null && templetChannel.getId()!=null && templetChannel.getId().trim().length()>0) {
				...
				log(getLoginName(), "删除模板栏目 "+templetChannel.getName(), getHttpRequest());
				...
				if (templetChannel.getId()!=null && templetChannel.getId().trim().length()>0
					...
					if (partempletChannel!=null) {
						...
						if (templetChannel!=null) {
							...
							log(getLoginName(), "改变模板栏目 "+templetChannel.getName()+" 的所属栏目为 "+partempletChannel.getName(), getHttpRequest());
							write("操作成功", "UTF-8");
						}
					}
					...
					if (oldtempletChannel!=null && templet!=null) {
						...
						log(getLoginName(), "改变模板栏目 "+oldtempletChannel.getName()+" 的所属栏目为 "+templet.getName()+" 模板的一级栏目", getHttpRequest());
						write("操作成功", "UTF-8");
					}
				}
			}
	...
	public String editDo(){
		try {
			...
			if (templetChannel.getId()!=null && templetChannel.getId().trim().length()>0) {
				...
				templetChannelService.update(templetChannel);
				log(getLoginName(), "更新模板栏目 "+templetChannel.getName(), getHttpRequest());
			}else {
				//添加
				//判断页面标识是否存在
				if (templetChannel.getPagemark()!=null && templetChannel.getPagemark().trim().length()>0 && 
				...
				log(getLoginName(), "添加模板栏目 "+templetChannel.getName(), getHttpRequest());
			}
