...
import cn.freeteam.service.impl.RoleServiceImpl;
import cn.freeteam.service.impl.RoleUserServiceImpl;
import cn.freeteam.service.impl.UnitServiceImpl;
import cn.freeteam.service.impl.UnitUserServiceImpl;
...
public class UserAction extends BaseAction{
	...
	public String pwd(){
		...
		log(users.getLoginname(), msg, getHttpRequest());
		return "pwd";
	}
	/**
	 * 个人资料页面
							...
							if (roleids[i].trim().length()>0) {
								if (!roleUserService.haveRoleUser(roleids[i], user.getId())) {
									...
									if (role!=null) {
										...
										log(getLoginName(), "添加角色人员关联("+role.getName()+" "+user.getLoginname()+")成功", getHttpRequest());
									}
								}
							}
						}
							...
							if (unitids[i].trim().length()>0) {
								if (!unitUserService.haveUnitUser(unitids[i], user.getId())) {
									...
									if (unit!=null) {
										...
										log(getLoginName(), "添加单位人员关联("+unit.getName()+" "+user.getLoginname()+")成功", getHttpRequest());
									}
								}
							}
						}
						...
						for (int i = 0; i < roleids.length; i++) {
							if (roleids[i].trim().length()>0) {
								...
								if (!roleUserService.haveRoleUser(roleids[i], user.getId())) {
									//无则添加
									roleUserService.add(roleids[i], user.getId());
									log(getLoginName(), "给人员"+authUser.getName()+"("+authUser.getLoginname()+")"+"授权("+rolenames[i]+")", getHttpRequest());
								}
							}
						}
					}
					if (delRoleids!=null && delRoleids.length>0) {
						...
						for (int i = 0; i < delRoleids.length; i++) {
							if(delRoleids[i].trim().length()>0){
								...
								log(getLoginName(), "人员授权删除("+user.getLoginname()+" "+delRole[1]+")", getHttpRequest());
							}
						}
					}
					//设置用户的角色名称
	...
	public String del(){
		...
		log(getLoginName(), logContent, getHttpRequest());
		write(msg, "UTF-8");
		return null;
	}
								...
								if (unitids[i].trim().length()>0) {
									if (!unitUserService.haveUnitUser(unitids[i], obj.getId())) {
										...
										if (unit!=null) {
											...
											log(getLoginName(), "编辑人员时添加单位人员关联("+unit.getName()+" "+obj.getLoginname()+")成功", getHttpRequest());
										}
									}
								}
							}
				...
				try {
					...
					for (int i = 0; i < idArr.length; i++) {
						...
						if (idArr[i].trim().length()>0) {
							...
							paramMap.put("id", idArr[i].trim());
							paramMap.put("state", state);
							usersMapper.updateById(paramMap);
						}
					}
					msg="1";
	...
	public void setRoleUserService(RoleUserServiceImpl roleUserService) {
